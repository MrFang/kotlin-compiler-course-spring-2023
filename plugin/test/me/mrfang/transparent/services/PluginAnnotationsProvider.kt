...
class PluginAnnotationsProvider(testServices: TestServices) : EnvironmentConfigurator(testServices) {
    companion object {
        ...
        private val ANNOTATIONS_JAR_FILTER = FilenameFilter { _, name -> name.startsWith("annotations") && name.endsWith(".jar") }
    }
    ...
    private val failMessage = { "Jar with annotations does not exist. Please run :annotations:jar" }
}
